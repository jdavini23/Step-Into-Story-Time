...
export function useErrorHandler(): UseErrorHandlerReturn {
  ...
  const categorizeError = useCallback((error: any): ErrorDetails => {
    ...
    if (error.message?.includes('validation') || error.statusCode === 400) {
      return {
        message: error.details ? 'Please check your input.' : error.message || 'Invalid input.',
  ...
  const handleError = useCallback((error: Error | ErrorDetails | any) => {
    ...
    const shouldShowToast = !['AUTHENTICATION_ERROR'].includes(errorDetails.code || '');
