    ...
    (req: any, res, next) => {
      ...
      if (!storyGenerationLimiter.isAllowed(userId)) {
        throw new RateLimitError(
          "Too many story generation requests. Please wait a moment.",
        );
      }
      next();
    },
    ...
    asyncHandler(async (req: any, res) => {
      ...
      try {
        ...
        if (error.message?.includes("OpenAI")) {
          throw new AppError(
            "Story generation service is temporarily unavailable",
            503,
            "SERVICE_UNAVAILABLE",
          );
        }
    ...
    asyncHandler(async (req: any, res) => {
      ...
      try {
        ...
        if (story.content && typeof story.content !== "string") {
          console.log(
            `Converting story ${storyId} content from ${typeof story.content} to string`,
          );
